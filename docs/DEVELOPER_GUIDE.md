# Developer Guide: Sovereign AutoPackager
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/autopackager.git
cd autopackager
...
```bash
export SBB_TEMPLATE_REGISTRY_URL=your_value_here
export SBB_GITHUB_TOKEN=your_value_here
export SBB_OUTPUT_DIR=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-autopackager)
...
```bash
# Build
docker build -t sovereign-autopackager:dev .

# Run with env file
docker run --env-file .env -p 8797:8797 sovereign-autopackager:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-autopackager
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8797`
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8797` in use | `lsof -i :8797` | Kill PID or change `SERVICE_PORT` |
